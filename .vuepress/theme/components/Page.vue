  ...
  <main class="page" :style="pageStyle">
    <ModuleTransition delay="0.08">
      <section v-show="recoShowModule">
        <div v-if="JSON.stringify(contentStyle) === '{}'" class="page-title">
          <h1 class="title">{{$page.title}}</h1>
          <PageInfo :pageInfo="$page" :showAccessNumber="showAccessNumber"></PageInfo>
        </div>
        <!-- 这里使用 v-show，否则影响 SSR -->
        <Content :style="contentStyle" class="theme-reco-content" />
      </section>
    </ModuleTransition>
...
export default defineComponent({
  ...
  setup (props, ctx) {
    ...
    const contentStyle = computed(() => {
      const categories = instance.$page.frontmatter.categories.length>0 ? instance.$page.frontmatter.categories[0] : ''
      let style = {}
      if(categories === '前端技术文档'){
        style = {
          maxWidth : 'auto',
          margin : '0',
          padding : 0
